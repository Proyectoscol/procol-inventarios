...
export async function GET(
  ...
  try {
    ...
    // Verificar que el cliente pertenezca a la compañía
    if (customer.companyId !== companyId) {
      return NextResponse.json(
        { error: "El cliente no pertenece a esta compañía" },
        { status: 403 }
      )
    }
    ...
    const nombreDestinatario = customer.nombreRecibe || customer.name
    ...
    if (customer.cedula) {
      doc.setFont('helvetica', 'normal')
      doc.text(`Cédula: ${customer.cedula}`, rightColumnX, yPosition)
      yPosition += 4.5
    }

    // Teléfono
    if (customer.phone) {
      doc.text(`Teléfono: ${customer.phone}`, rightColumnX, yPosition)
      yPosition += 4.5
    }

    // Dirección estructurada
    if (customer.departamento || customer.ciudad) {
      const ubicacion = [customer.departamento, customer.ciudad].filter(Boolean).join(", ")
      doc.text(ubicacion, rightColumnX, yPosition)
      yPosition += 4.5
    }

    if (customer.barrio) {
      doc.text(`Barrio: ${customer.barrio}`, rightColumnX, yPosition)
      yPosition += 4.5
    }

    if (customer.direccion1) {
      const dir1Lines = doc.splitTextToSize(customer.direccion1, columnWidth - 5)
      doc.text(dir1Lines, rightColumnX, yPosition)
      yPosition += dir1Lines.length * 4.5
    }

    if (customer.direccion2) {
      const dir2Lines = doc.splitTextToSize(customer.direccion2, columnWidth - 5)
      doc.text(dir2Lines, rightColumnX, yPosition)
      yPosition += dir2Lines.length * 4.5
    }

    // Si no hay dirección estructurada, usar address legacy
    if (!customer.direccion1 && customer.address) {
      const addressLines = doc.splitTextToSize(customer.address, columnWidth - 5)
    ...
    doc.setFontSize(7)
    doc.setFont('helvetica', 'normal')
    // No hay número de orden ni fecha de movimiento para guías generadas desde cliente
    ...
    return new NextResponse(uint8Array, {
      headers: {
        ...
        'Content-Disposition': `inline; filename="guia-envio-${customer.name || 'cliente'}.pdf"`,
