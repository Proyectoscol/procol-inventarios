...
export function ProductSearchWithWarehouse({ 
  ...
  const renderItems = () => {
    if (warehouseId) {
      ...
      // Ordenar: con stock primero (desc), luego sin stock
      ...
      const items = filteredProducts.map((product) => {
        ...
        return { product, qty, productKey }
      }).filter(Boolean) as Array<{ product: ProductWithWarehouse; qty: number; productKey: string }>
      ...
      return sorted.map(({ product, qty, productKey }) => {
        const noStock = qty === 0
        return (
          <button
            key={productKey}
          ...
          >
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className={`text-xs ${noStock ? "text-red-500" : "text-green-700 font-medium"}`}>
                {noStock ? "Sin stock" : `Stock: ${qty}`}
              </span>
            </div>
          </button>
        )
